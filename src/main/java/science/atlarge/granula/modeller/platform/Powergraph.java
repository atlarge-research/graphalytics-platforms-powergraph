...
import science.atlarge.granula.modeller.platform.operation.*;
import science.atlarge.granula.modeller.rule.extraction.PowergraphExtractionRule;
...
public class Powergraph extends PlatformModel {

    public Powergraph() {
        super();
        addOperationModel(new PowergraphJob());
    ...
    protected class JobInfoRule extends DerivationRule {
        ...
        public boolean execute() {
            ...
            platform.setName("A Powergraph job");
            platform.setType("Powergraph");
            ...
            try {
                Operation processGraph = platform.findOperation(Type.PowerGraph, Type.ProcessGraph);
                long processingTime = Long.parseLong(processGraph.getInfo("Duration").getValue());

                Operation loadGraph = platform.findOperation(Type.PowerGraph, Type.LoadGraph);
                long loadTime = Long.parseLong(loadGraph.getInfo("Duration").getValue());

                Operation topOperation = platform.findOperation(Type.PowerGraph, Type.Job);
